...
public class MethodComparator<T> implements Comparator<T> {
    ...
    public MethodComparator(Class<? extends T> clazz, String methodName, Order order) {
        ...
        for (Method method : methods) {
            ...
            if (method.getName().equals(methodName) && Comparable.class.isAssignableFrom(method.getReturnType())
        ...
        if (method == null) {
            ...
            throw new RuntimeException("No method named " + methodName
                    + " returning a comparable class and taking no paramaters found in " + clazz);
        }
