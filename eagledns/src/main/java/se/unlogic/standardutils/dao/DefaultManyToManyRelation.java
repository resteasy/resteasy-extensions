...
public class DefaultManyToManyRelation<LocalType, RemoteType> implements ManyToManyRelation<LocalType, RemoteType> {
    ...
    public DefaultManyToManyRelation(Class<LocalType> beanClass, Class<RemoteType> remoteClass, Field field,
            AnnotatedDAOFactory daoFactory, DAOManaged daoManaged) {
        super();
        ...
        for (Field remoteField : fields) {
            ...
            if (ReflectionUtils.getGenericlyTypeCount(remoteField) == 1
                    && ReflectionUtils.getGenericType(remoteField).equals(beanClass)
                    && remoteField.isAnnotationPresent(DAOManaged.class) && remoteField.isAnnotationPresent(ManyToMany.class)
                    && remoteField.getAnnotation(ManyToMany.class).linkTable().equals(linkTable)) {
        ...
        if (matchingRemoteField == null) {
            ...
            throw new RuntimeException("Unable to to find corresponding @ManyToMany field in " + remoteClass
                    + " while parsing field " + field.getName() + " in " + beanClass);
        }
        ...
        this.linkTableInsertSQL = "INSERT INTO " + linkTable + " (" + localLinkTableColumnName + "," + remoteLinkTableColumnName
                + ") VALUES (?,?)";
    ...
    private void init() {
        ...
        if (this.linkTableLinkSQL == null) {
            this.linkTableLinkSQL = "SELECT " + annotatedDAO.getTableName() + ".* FROM " + annotatedDAO.getTableName()
                    + " INNER JOIN " + linkTable + " ON (" + annotatedDAO.getTableName() + "." + remoteKeyColumnName + "="
                    + linkTable + "." + remoteLinkTableColumnName + ") WHERE " + linkTable + "." + localLinkTableColumnName
                    + " = ?";
        }
    ...
    private static <Type> void setQueryParameter(UpdateQuery query, Column<Type, ?> column, Type bean, int index)
            throws SQLException {
    ...
    public static Field getKeyField(ManyToMany manyToManyAnnotation, Class<?> clazz, Field annotatedField) {
        ...
        if (!StringUtils.isEmpty(manyToManyAnnotation.keyField())) {
            ...
            try {
                ...
                if (keyDAOPopulate == null) {
                    ...
                    throw new RuntimeException(
                            "Specified keyField " + manyToManyAnnotation.keyField() + " for @ManyToMany annotation for field "
                                    + annotatedField.getName() + "  in " + clazz + " is missing the @DAOManaged annotation");
                }
                ...
                throw new RuntimeException("Unable to find specified keyField " + manyToManyAnnotation.keyField()
                        + " for @ManyToMany annotation for field " + annotatedField.getName() + "  in " + clazz);
            ...
            if (keyFields.size() == 0) {
                ...
                throw new RuntimeException(
                        "keyField needs to be specified for @ManyToMany annotated field " + annotatedField.getName() + " in "
                                + clazz + " since the class contains multiple @Key annotated fields");
            }
    ...
    public static <LT, RT> ManyToManyRelation<LT, RT> getGenericInstance(Class<LT> beanClass, Class<RT> remoteClass,
            Field field, AnnotatedDAOFactory daoFactory, DAOManaged daoManaged) {
