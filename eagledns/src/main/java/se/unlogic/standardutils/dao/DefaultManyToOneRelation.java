...
public class DefaultManyToOneRelation<LocalType, RemoteType, RemoteKeyType>
    ...
    public DefaultManyToOneRelation(Class<LocalType> beanClass, Class<RemoteType> remoteClass,
            Class<RemoteKeyType> remoteKeyClass, Field field, Field remoteKeyField, DAOManaged daoManaged,
        ...
        if (resultSetMethod != null) {
            ...
            BeanStringPopulator<RemoteKeyType> typePopulator = BeanStringPopulatorRegistery
                    .getBeanStringPopulator(remoteKeyClass);
            ...
            if (typePopulator != null) {
                ...
                throw new UnsupportedFieldTypeException("Unable to find resultset method or type populator for field "
                        + remoteKeyField.getName() + " in " + remoteClass + " when creating many to one relation for field "
                        + field.getName() + " in " + beanClass, field, ManyToOne.class, beanClass);
            }
        }
    }
     ...
     * (non-Javadoc)
     *
     * @see se.unlogic.utils.dao.ManyToOneRelation#setValue(LocalType, java.sql.ResultSet, java.sql.Connection,
     * java.lang.reflect.Field[])
     */
    public void getRemoteValue(LocalType bean, ResultSet resultSet, Connection connection, RelationQuery relationQuery)
            throws SQLException {
    ...
    public static <LT, RT, RKT> DefaultManyToOneRelation<LT, RT, RKT> getGenericInstance(Class<LT> beanClass,
            Class<RT> remoteClass, Class<RKT> remoteKeyClass, Field field, Field remoteField, DAOManaged daoManaged,
            AnnotatedDAOFactory daoFactory) {

        return new DefaultManyToOneRelation<LT, RT, RKT>(beanClass, remoteClass, remoteKeyClass, field, remoteField, daoManaged,
                daoFactory);
    }
}
