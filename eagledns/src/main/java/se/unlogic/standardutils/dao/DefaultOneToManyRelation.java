...
public class DefaultOneToManyRelation<LocalType, RemoteType> implements OneToManyRelation<LocalType, RemoteType> {
    ...
    public DefaultOneToManyRelation(Class<LocalType> beanClass, Class<RemoteType> remoteClass, Field field,
        ...
        for (Field remoteField : fields) {
            ...
            if (remoteField.getType().equals(beanClass) && remoteField.isAnnotationPresent(DAOManaged.class)
                    && remoteField.isAnnotationPresent(ManyToOne.class)) {
        ...
        if (this.remoteField == null) {
            ...
            throw new RuntimeException("Unable to to find corresponding @ManyToOne field in class " + remoteClass
                    + " for @OneToMany annotated field " + field.getName() + " in " + beanClass);
        }
    }
    ...
    private void fixReferences(List<RemoteType> remoteBeans, LocalType bean)
    ...
    public static <LT, RT> OneToManyRelation<LT, RT> getGenericInstance(Class<LT> beanClass, Class<RT> remoteClass, Field field,
            AnnotatedDAOFactory daoFactory, DAOManaged daoManaged) {
