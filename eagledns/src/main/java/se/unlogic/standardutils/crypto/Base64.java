 ...
 * such as encodeBytes( bytes, options ) the options parameter can be used to indicate such things as first gzipping the bytes
 ...
 * line feeds unless explicitly told
 ...
 * but should not throw an
 * ArrayIndexOutOfBoundsException either. Led to discovery of mishandling (or potential for better handling) of other bad input
 * characters. You should now get
 ...
 * (by padding with equals signs)
 * too soon. Also added an option to suppress the automatic decoding of gzipped streams. Also added experimental support for
 * specifying a class loader when
 ...
 * forth. Fixed some javadocs that
 ...
 * code doesn't have to create two
 * output arrays: an oversized initial one and then a final, exact-sized one. Big win when using the
 * {@link #encodeBytesToBytes(byte[])} family of methods (and
 ...
 * with memory by not returning a
 * String but just a byte array.</li>
 * <li>v2.3 - <strong>This is not a drop-in replacement!</strong> This is two years of comments and bug fixes queued up and
 * finally executed. Thanks to everyone
 * who sent me stuff, and I'm sorry I wasn't able to distribute your fixes to everyone else. Much bad coding was cleaned up
 ...
 * the GZIP option) use IO streams,
 * there is a possiblity of an java.io.IOException being thrown. After some discussion and thought, I've changed the behavior of
 * the methods to throw
 * java.io.IOExceptions rather than return null if ever there's an error. I think this is more appropriate, though it will
 * require some changes to your code.
 ...
 * lexical ordering as described in
 ...
 * now detects when data that's
 * being decoded is gzip-compressed and will decompress it automatically. Generally things are cleaner. You'll probably have to
 * change some method calls that
 ...
 * Added the ability to "suspend"
 * encoding in the Output Stream so you can turn on and off the encoding if you need to embed base64 data in an otherwise
 * "normal" stream (like an XML file).</li>
 * <li>v1.5 - Output stream pases on flush() command but doesn't do anything itself. This helps when using GZIP streams. Added
 * the ability to GZip-compress
 ...
 * with plenty of well-wishing
 * instead! Please visit <a href="http://iharder.net/base64">http://iharder.net/base64</a> periodically to check for updates or
...
public class Base64 {
     ...
     * encoded this way is
     * <em>not</em> officially valid Base64, or at the very least should not be called Base64 without also specifying that is
     * was encoded using the URL- and
     ...
     * specify ORDERED <b>and</b> URLSAFE
     ...
     * specify ORDERED and URL_SAFE in
     ...
     * actual number of significant
     * bytes in your array is given by <var>numSigBytes</var>. The array <var>threeBytes</var> needs only be as big as
     * <var>numSigBytes</var>. Code can reuse a
     ...
     * <var>destination</var>. The source and destination
     * arrays can be manipulated anywhere along their length by specifying <var>srcOffset</var> and <var>destOffset</var>. This
     * method does not check to make
     * sure your arrays are large enough to accomodate <var>srcOffset</var> + 3 for the <var>source</var> array or
     ...
     * versions, it just returned a null
     ...
     * efficient if you're working with I/O
     ...
     * <var>destination</var>. The source and
     * destination arrays can be manipulated anywhere along their length by specifying <var>srcOffset</var> and
     * <var>destOffset</var>. This method does not
     * check to make sure your arrays are large enough to accomodate <var>srcOffset</var> + 4 for the <var>source</var> array or
     * <var>destOffset</var> + 3 for
     ...
     * set.</strong> This is not generally a
     * recommended method, although it is used internally as part of the decoding process. Special case: if len = 0, an empty
     * array is returned. Still, if you
     ...
     * <tt>loader</tt> is not null, it will
     ...
     * versions, it just returned false,
    ...
    public static class OutputStream extends java.io.FilterOutputStream {
         ...
         * three at a time before the output
